...
props.globals.initNode("instrumentation/radar/range", 100, "INT");
...
var SwpFac            = props.globals.initNode("instrumentation/radar-awg-9/sweep-factor", 1, "DOUBLE");
var DisplayRdr        = props.globals.initNode("instrumentation/radar-awg-9/display-rdr", 1, "DOUBLE");
var HudTgtHDisplay    = props.globals.initNode("instrumentation/radar-awg-9/hud/target-display", 1, "DOUBLE");
var HudTgt            = props.globals.initNode("instrumentation/radar-awg-9/hud/target", 1, "DOUBLE");
var HudTgtTDev        = props.globals.initNode("instrumentation/radar-awg-9/hud/target-total-deviation", 1, "DOUBLE");
var HudTgtTDeg        = props.globals.initNode("instrumentation/radar-awg-9/hud/target-total-angle", 1, "DOUBLE");
var HudTgtClosureRate = props.globals.initNode("instrumentation/radar-awg-9/hud/closure-rate", 1, "DOUBLE");
var WcsMode           = props.globals.initNode("instrumentation/radar-awg-9/wcs-mode", 1, "DOUBLE");
var HudTgtDistance    = props.globals.initNode("instrumentation/radar-awg-9/hud/distance", 1, "DOUBLE");

var AzField           = props.globals.initNode("instrumentation/radar-awg-9/az-field", 120, "INT");
var RangeRadar2       = props.globals.initNode("instrumentation/radar-awg-9/radar2-range", 40, "INT");
var RadarStandby      = props.globals.initNode("instrumentation/radar-awg-9/radar-standby", 40, "INT");

var SWTgtRange        = props.globals.initNode("systems/armament/aim9/target-range-nm", 10, "INT");

var EcmOn             = props.globals.initNode("instrumentation/ecm/on-off", 1, "BOOL");
var EcmAlert1         = props.globals.initNode("instrumentation/ecm/alert-type1", 0, "BOOL");
var EcmAlert2         = props.globals.initNode("instrumentation/ecm/alert-type2", 0, "BOOL");
...
var az_scan = func() {
	...
	foreach( u; tgts_list ) {
			...
			or ( ! swp_dir and swp_deg <= u.deviation and u.deviation < swp_deg_last ))
        {
			...
			if ( 1)#u_rng < horizon and radardist.radis(u.string, my_radarcorr))
            {
...
Diamond_Blinker = aircraft.light.new("lighting/hud-diamond-switch", [0.1, 0.1]);
setprop("lighting/hud-diamond-switch/enabled", 1);
...
wcs_mode_sel = func(mode) {
	if ( pilot_lock and ! we_are_bs ) { return }
	foreach (var n; props.globals.getNode("instrumentation/radar-awg-9/wcs-mode").getChildren()) {
...
var Target = {
	new : func (c) {
		...
		obj.InstrTgts = props.globals.getNode("instrumentation/radar-awg-9/targets", 1);
		if ( bs == "f-14b-bs") {
			if  ( BS_instruments.Pilot != nil ) {
				# Use a different radar-awg-9 folder.
				obj.InstrTgts = BS_instruments.Pilot.getNode("instrumentation/radar-awg-9/targets", 1);
...
#		}
    ...
    get_Coord: func(){
        if (me.lat != nil) {
            ...
            if (me.x != nil)
            {
                me.TgTCoord.set_xyz(me.x.getValue(),
                                    me.y.getValue(),
                                    me.z.getValue());
            }
            else {
